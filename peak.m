...
CONFIG.SAFE_DISTANCE_MIN = 1.5; % 最小安全距离(m)
CONFIG.HEADWAY_TIME = 1.5; % 车头时距(s)
CONFIG.CAR_TO_BIKE_RATIO = 4; % 汽车与自行车生成比例
CONFIG.SPAWN_ATTEMPT_LANES = 4;  % 每帧最多尝试的入口车道数
...
function vehicles = spawnVehicles(vehicles, model, roads, time)
    ...
    numLanesToTry = min(CONFIG.SPAWN_ATTEMPT_LANES, numel(carGenLaneIndices));
    selectedLanes = carGenLaneIndices(randperm(numel(carGenLaneIndices), numLanesToTry));

    for k = 1:numel(selectedLanes)
        % 每个时间步仅尝试生成一次车辆
